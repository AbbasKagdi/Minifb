        ...
        <div class="col-md-6">
            ...
            <form action="/listings" method="POST" enctype="multipart/form-data">
                ...
                <div class="form-group my-3">
                    ...
                    <input type="text" class="form-control" name="company" id="company" placeholder="Mckinsey & Co" value="{{old('company')}}">
                    @error('company') <small class="text-danger my-1">{{$message}}</small> @enderror
                </div>
                <div class="form-group my-3">
                    <label for="title">Job Title</label>
                    <input type="text" class="form-control" name="title" id="title" placeholder="Senior Laravel Developer" value="{{old('title')}}">
                    @error('title') <small class="text-danger my-1">{{$message}}</small> @enderror
                </div>
                <div class="form-group my-3">
                    <label for="location">Job Location</label>
                    <input type="text" class="form-control" name="location" id="location" placeholder="Remote / Nebraska" value="{{old('location')}}">
                    @error('location') <small class="text-danger my-1">{{$message}}</small> @enderror
                </div>
                <div class="form-group my-3">
                    <label for="email">Corporate Email</label>
                    <input type="email" class="form-control" name="email" id="email" placeholder="https://www.mckinsey.com/" value="{{old('email')}}">
                    @error('email') <small class="text-danger my-1">{{$message}}</small> @enderror
                </div>
                <div class="form-group my-3">
                    <label for="websiite">Website / Application URL</label>
                    <input type="url" class="form-control" name="website" id="website" placeholder="https://www.mckinsey.com/" value="{{old('website')}}">
